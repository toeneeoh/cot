...
OnInit.final("Death", function(Require)
    ...
    do
        ...
        function thistype:onCast()
            BlzSetSpecialEffectScale(HeroReviveIndicator[self.pid], 0)
            DestroyEffect(HeroReviveIndicator[self.pid])
            BlzSetSpecialEffectScale(HeroTimedLife[self.pid], 0)
            DestroyEffect(HeroTimedLife[self.pid])
            TimerList[self.pid]:stopAllTimers('dead')

            --item revival
            if self.sid == FourCC('A042') then
                local itm2 = GetResurrectionItem(self.pid, false)
                local heal = 0
...
function BossHandler(uid)
    ...
    if uid == FourCC('E00B') or uid == FourCC('E00D') or uid == FourCC('E00C') then
        DeadGods = DeadGods + 1
...
function OnDeath()
    ...
    elseif killed == Hero[pid] then
        ...
        if BlzGetUnitAbilityCooldownRemaining(killed, RESURRECTION.id) <= 0 and GetUnitAbilityLevel(killed, RESURRECTION.id) > 0 then
            UnitAddAbility(HeroGrave[pid], RESURRECTION.spell)
    ...
    elseif index ~= -1 and p == pboss and spawnflag then
        ...
        --reset total damage
        BossTable[index].total_damage = 0
